defmodule Elasticlunr.Benchmark.Downloader do
  @moduledoc false
  ...
  * `{ :error, :eexist }` if file exists already
  ...
  ## Examples
      iex> Downloader.download("http://speedtest.ftp.otenet.gr/files/test100k.db")
      { :ok, "/absolute/path/to/test_100k.db" }
      iex> Downloader.download("http://speedtest.ftp.otenet.gr/files/test100k.db", [max_file_size: 99 * 1000])
      { :error, :file_size_is_too_big }
      iex> Downloader.download("http://speedtest.ftp.otenet.gr/files/test100k.db", [path: "/custom/absolute/file/path.db"])
  ...
  @spec download(binary(), keyword()) ::
          {:ok, binary()} | {:error, :file_size_is_too_big | :eexist | :download_failure}
  ...
  defp create_file(path) do
    case File.exists?(path) do
      true ->
        File.open(path, ~w[write exclusive]a)
